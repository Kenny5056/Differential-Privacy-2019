function [trainX,trainY,testX,testY] = LSplusDeepLearningCreation(W,data,epsilon,gamma,trainsetsize,testsetsize)

index = randsample(1:length(data),trainsetsize+testsetsize);
randdata = data(index,:);
train = randdata(1:trainsetsize,:);
testsetX = randdata(trainsetsize+1:end,1:size(data,2)-1);
testY = randdata(trainsetsize+1:end,size(data,2));

noisytrain = zeros(size(train));
m = length(train);
w = length(W);

%Waveletblock
for i = 1:m/w
block = train(1+w*(i-1):w*i,:);
...
trainX = noisytrain(:,1:size(noisytrain,2)-1);
trainY = noisytrain(:,size(noisytrain,2));
testX = noisytest;

csvwrite('trainX82.csv',trainX);
csvwrite('trainY82.csv',trainY);
csvwrite('testX82.csv',testX);
csvwrite('testY82.csv',testY);
end
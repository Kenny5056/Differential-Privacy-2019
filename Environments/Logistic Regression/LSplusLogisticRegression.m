function [averagepercentage] = LSplusLogisticRegression(W,data,epsilon,gamma,trials,trainsetsize,testsetsize)
...
noisyblockresp = noisyblockresp + ones(size(noisyblockresp));
...
%Testing set
B = mnrfit(noisytrainX,noisytrainY);
pihat = mnrval(B,noisytest);

newpihat = zeros(length(noisytest),1);
for iii = 1:length(noisytest)
    if pihat(iii,1) >= 0.5
        newpihat(iii,1) = 0;
    else
        newpihat(iii,1) = 1;

function [averagepercentage,noisytrain,trainset] = PQClassicalNN(W,data,epsilon,trials,testsetsize)
...
for k = 1:size(trainset,2)
    for L = 1:size(trainset,1)/3
        theta(L,k) = (pi*((A(L,k) + mu - 2*v)))/(6*(mu - v));
    end
end

X = laplace(size(trainset,1)/3,size(trainset,2),0,2/epsilon);

mu2 = max(X,[],'all');
v2 = min(X,[],'all');
alpha = pi.*((X + mu2 - 2*v2)/(6*(mu2 - v2)));

K = rand(size(trainset,1)/3,size(trainset,2));

thetae = zeros(size(X));

for ii=1:size(trainset,1)/3
    for jj=1:size(trainset,2)
        if K(ii,jj) >= 0.5
...
Astar = zeros(size(A));
for m = 1:size(trainset,1)/3
    for n = 1:size(trainset,2)
        Astar(m,n) = (6/pi)*(mu-v)*thetae(m,n) - mu + 2*v;
    end
end

Bstar = [Astar; B(size(trainset,1)/3 + 1:end,:)];
noisytrain = W' * Bstar;
...
%The very obvious outliers are the ones with (exactly) 0% classification accuracy
outliers = matrix(:,:) == 0;
matrix(outliers,:) = [];
